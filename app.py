...
def create_cookie_file():
    ...
    with open(cookie_file, 'w', encoding='utf-8') as f:
        ...
        # Format: HOST\tDOMAIN_SPECIFIED\tPATH\tSECURE\tEXPIRY\tNAME\tVALUE
        expiry = int(datetime.now().timestamp()) + 31536000  # 1 year from now
        
        # Add YouTube consent cookies with proper format
        # Note: TRUE/FALSE for domain_specified indicates if domain starts with a dot
        f.write(f'.youtube.com\tTRUE\t/\tFALSE\t{expiry}\tCONSENT\t{YOUTUBE_CONSENT}\n')
        
    # Verify the cookie file is created and readable
    ...
    try:
        with open(cookie_file, 'r', encoding='utf-8') as f:
            ...
            if not content or 'youtube.com' not in content:
                raise Exception("Cookie file content verification failed")
    except Exception as e:
        raise Exception(f"Cookie file verification failed: {str(e)}")
